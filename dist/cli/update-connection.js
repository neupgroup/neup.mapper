...
try {
    ...
    const connectionIndex = connections.findIndex((c) => c.name === connectionName);
    ...
    if (typeArg) {
        ...
        // Normalize type
        if (['postgresql', 'postgre', 'postgres'].includes(newType))
            newType = 'postgres';
        else if (['mariadb', 'mysql'].includes(newType))
            newType = 'mysql';
        else if (['sqlitedb', 'sqlite'].includes(newType))
            newType = 'sqlite';
