            ...
            try {
                ...
                if (schemaDef && schemaDef.fields) {
                    ...
                    if (schemaDef.insertableFields)
                        options.insertableFields = schemaDef.insertableFields;
                    if (schemaDef.updatableFields)
                        options.updatableFields = schemaDef.updatableFields;
                    if (schemaDef.massUpdateable !== undefined)
                        options.massEditAllowed = schemaDef.massUpdateable;
                    if (schemaDef.massDeletable !== undefined)
                        options.massDeleteAllowed = schemaDef.massDeletable;
                    if (Object.keys(options).length > 0) {
                        ...
                        const def = builder.mapper.getSchemaManager().use(schemaName).def;
                        if (def) {
                            if (options.insertableFields)
                                def.insertableFields = options.insertableFields;
                            if (options.updatableFields)
                                def.updatableFields = options.updatableFields;
                            if (options.massEditAllowed !== undefined)
                                def.massEditAllowed = options.massEditAllowed;
                            if (options.massDeleteAllowed !== undefined)
                                def.massDeleteAllowed = options.massDeleteAllowed;
                        }
                    }
                }
            }
