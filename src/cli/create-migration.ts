...
function createMigrationString(id: string, name: string, conn: string | null): string {
  return `    "${id}": {
      id: "${id}",
      name: "${name}",
...
if (fs.existsSync(tsConfigPath)) {
  ...
  if (config.migrations.some((m: any) => m.id === fullName)) {
  ...
  if (connectionName) newMigration.connection = connectionName;
